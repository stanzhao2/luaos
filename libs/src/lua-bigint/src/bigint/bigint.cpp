...
big_integer gcd(big_integer const& a, big_integer const& b)
{
  if (a == 0) {
    return b;
  }
...
big_integer pow(big_integer const& a, big_integer const& b)
{
  if (b == 0) {
    return 1;
  }
...
int string_to_int(std::string const &s) {
  ...
  for (auto a : s) {
    if (a < '0' || a > '9') {
      throw std::runtime_error("string is not number");
    }
    ans = ans * 10 + (a - '0');
  }
  return ans;
...
big_integer operator/(big_integer const &a, big_integer const &b) {
  ...
  if (bLength == 1) {
    ...
    for (int k = (int)(aLength - bLength); k >= 0; k--) {
