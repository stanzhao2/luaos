...
//#include "config.h"
//#include "D:\\projects\\luaos\\libffi-3.4.3\\msvc_build\\aarch64\\aarch64_include\\ffi.h"
...
static int alien_function_call(lua_State *L) {
  ...
  case AT_string: ffi_call(cif, af->fn, &pret, args);
    //(pret ? lua_pushstring(L, (const char *)(char*)pret) : lua_pushnil(L)); break;
	if (pret){
...
static int alien_table_new(lua_State *L) {
  int narray = (int)luaL_optinteger(L, 1, 0);
  int nhash = (int)luaL_optinteger(L, 2, 0);
