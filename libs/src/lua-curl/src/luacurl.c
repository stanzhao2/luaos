...
static int lexnew_metatable(lua_State* L, const char* name, const luaL_Reg* methods)
{
...
static size_t readerCallback(void* ptr, size_t size, size_t nmemb, void* stream)
{
  ...
  lua_pushnumber(c->L, size * (lua_Number)nmemb);
  lua_call(c->L, 2, 1);
...
static size_t writerCallback(void* ptr, size_t size, size_t nmemb, void* stream)
{
  ...
  lua_pushlstring(c->L, (char*)ptr, size * nmemb);
  lua_call(c->L, 2, 1);
...
/* Expands all the list of switch-case's here */
  ...
  if (CURLE_OK == (code = curl_easy_setopt(c->curl, (CURLoption)(int)lua_tonumber(L, 2), v.sval)))
  {
...
{
  lexnew_metatable(L, CURLHANDLE, luacurl_meths);
  lua_pop(L, 1);
}
...
static void set_info(lua_State* L)
{
  ...
  LUA_SET_TABLE(L, literal, "LIB_CURLVERSION", string, curl_version());
}
...
LUA_API int luaopen_curl(lua_State* L)
{
  ...
  lua_newtable(L);
  luaL_setfuncs(L, luacurl_funcs, 0);

  //luaL_register(L, 0, luacurl_meths);
  //luaL_register(L, 0, luacurl_funcs);
