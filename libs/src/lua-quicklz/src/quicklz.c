...
static int quicklz_compress(lua_State* L)
{
	size_t size = 0;
	const char* data = luaL_checklstring(L, 1, &size);
	qlz_state_compress* pst = (qlz_state_compress*)malloc(sizeof(qlz_state_compress));
	if (!pst) {
		return 0;
	}
	...
	size = qlz_compress(data, buffer, size, pst);
	free(pst);
...
static int quicklz_decompress(lua_State* L)
{
	size_t size = 0;
	const char* data = luaL_checklstring(L, 1, &size);
	if (qlz_size_compressed(data) != size) {
		return 0;
	}

	char temp[8192];
	char* buffer = temp;
	size_t decompress_size = qlz_size_decompressed(data);

	if (decompress_size > sizeof(temp)) {
		buffer = (char*)malloc(decompress_size);
		if (!buffer) {
			return 0;
		}
	}

	qlz_state_decompress* pst = (qlz_state_decompress*)malloc(sizeof(qlz_state_decompress));
	if (!pst) {
		if (buffer != temp) {
			free(buffer);
		}
	...
	size = qlz_decompress(data, buffer, pst);
	free(pst);
...
LUALIB_API int luaopen_quicklz(lua_State* L)
{
	struct luaL_Reg methods[] = {
		{ "compress",			quicklz_compress		},
		{ "decompress",   quicklz_decompress	},
		{ NULL,						NULL								}
	};
	lua_newtable(L);
