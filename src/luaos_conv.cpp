...
static int xor_convert(lua_State* L)
{
  std::string data;
  size_t slen, klen;
...
static int hash_md5(lua_State* L)
{
  ...
  MD5_CTX md5c;
  MD5_Init(&md5c);
  MD5_Update(&md5c, data.c_str(), data.size());
  MD5_Final(hash, &md5c);
  if (raw) {
...
static int hash_sha224(lua_State* L)
{
  ...
  SHA256_CTX sha1;
  SHA224_Init(&sha1);
  SHA224_Update(&sha1, data.c_str(), data.size());
...
static int hash_sha384(lua_State* L)
{
  ...
  SHA512_CTX sha1;
  SHA384_Init(&sha1);
  SHA384_Update(&sha1, data.c_str(), data.size());
...
static int hash_sha512(lua_State* L)
{
  ...
  SHA512_CTX sha1;
  SHA512_Init(&sha1);
  SHA512_Update(&sha1, data.c_str(), data.size());
...
int luaopen_openssl(lua_State* L)
{
  ...
  lua_newtable(L);
  struct luaL_Reg base64[] = {
    { "encode",       base64_encode     },
    { "decode",       base64_decode     },
    { NULL,		        NULL }
  };
  luaL_setfuncs(L, base64, 0);
