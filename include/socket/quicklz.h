...
#ifndef __QLZ_H_INCLUDED
#define __QLZ_H_INCLUDED
...
static __inline int32_t qlz_get_setting(int32_t setting)
{
    switch (setting)
    {
...
#if QLZ_COMPRESSION_LEVEL == 1
static int32_t same(const unsigned char *src, size_t n)
{
    while(n > 0 && *(src + n) == *src)
        n--;
    return n == 0 ? 1 : 0;
...
static __inline size_t qlz_size_decompressed(const char *source)
{
    ui32 n, r;
    n = (((*source) & 2) == 2) ? 4 : 1;
...
static __inline size_t qlz_size_compressed(const char *source)
{
    ui32 n, r;
    n = (((*source) & 2) == 2) ? 4 : 1;
...
static __inline size_t qlz_size_header(const char *source)
{
...
#if QLZ_COMPRESSION_LEVEL <= 2
static __inline void update_hash_upto(qlz_state_decompress *state, unsigned char **lh, const unsigned char *max)
{
    while(*lh < max)
    {
        (*lh)++;
...
static __inline size_t qlz_compress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_state_compress *state)
{
...
static __inline size_t qlz_decompress_core(const unsigned char *source, unsigned char *destination, size_t size, qlz_state_decompress *state, const unsigned char *history)
{
    ...
    for(;;) 
    {
        ...
        else
        {
            ...
            {
                uint32_t n = bitlut[cword_val & 0xf];
#ifdef X86X64
                *(ui32 *)dst = *(ui32 *)src;
#else
                memcpy_up(dst, src, 4);
...
static __inline size_t qlz_compress(const void *source, char *destination, size_t size, qlz_state_compress *state)
{
    size_t r;
    ui32 compressed;
    size_t base;
...
static __inline size_t qlz_decompress(const char *source, void *destination, qlz_state_decompress *state)
{
